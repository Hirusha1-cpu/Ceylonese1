  ...
  return (
    <div
      style={{
        ...
        right: "0",
        zIndex: "10",
        backgroundColor: scrolling ? "#830b0b" : "transparent", // Change background color when scrolling
        transition: "background-color 0.2s ease-in-out",
      }}
      id="navbar"
      className={`h-1/2 navbar  text-white ${
        scrolling ? "" : "bg-gradient-to-b from-black max-md:bg-red-950 hover:from-red-900"
      } p-4 flex justify-between items-center`}    >
      {/* logo */}
      <div className="uppercase font-bold text-4xl max-md:text-2xl">
        <Link href="/">Ceylonese</Link>
      </div>
      {/* left link */}
      {scrolling ? (
        <div className="hidden md:flex gap-4 pl-20">
          <Link href="/">Home</Link>
        ...
        <div
          className="hidden md:flex gap-4 pl-60"
          style={{
            transition: "max-width 0.5s ease-in-out", // Add transition to max-width
          }}
        >
          <Link href="/">Home</Link>
          <Link href="/menu">Menu</Link>
